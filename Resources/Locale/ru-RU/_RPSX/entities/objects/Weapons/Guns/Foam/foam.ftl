ent-WeaponSubMachineGunFoamTitmouse = синица
    .desc = Крупный и крутой пистолет-пулемет для пенной стрельбы со встроенным магазином большой емкости.

ent-WeaponSubMachineGunFoamFlash = вспышка
    .desc = Пенный пистолет-пулемет для скрытных спецопераций.

ent-WeaponPistolFoamHummingbird = колибри
    .desc = Пенный пистолет, который пригодится, когда закончатся другие боеприпасы.